...
# Repositories currently being downloaded (in-process tracking)
_downloading: set[str] = set()


def _models_dir_for(model_type: str) -> Path:
    if model_type == "embedding":
        return _EMBEDDING_MODELS_DIR
    return _CHAT_MODELS_DIR

# ---------------------------------------------------------------------------
# Curated lists of downloadable models
# ---------------------------------------------------------------------------
...
def _scan_models(base_dir: Path) -> List[ModelInfo]:
    ...
    for config_file in base_dir.rglob("config.json"):
        ...
        results.append(ModelInfo(
            name=model_dir.name,
            path=rel_str,
            size="Downloading..." if is_downloading else _human_readable_size(_dir_size(model_dir)),
...
async def _download_background(repository: str, model_type: str) -> None:
    ...
    base_dir = _models_dir_for(model_type)
...
class ModelService:
    ...
    def list_chat_models(self) -> List[ModelInfo]:
        return _scan_models(_CHAT_MODELS_DIR)

    def list_embedding_models(self) -> List[ModelInfo]:
        return _scan_models(_EMBEDDING_MODELS_DIR)

    def list_downloadable_chat_models(self) -> List[DownloadableModelInfo]:
        return _DOWNLOADABLE_CHAT_MODELS

    def list_downloadable_embedding_models(self) -> List[DownloadableModelInfo]:
        return _DOWNLOADABLE_EMBEDDING_MODELS

    async def download_model(self, repository: str) -> ModelDownloadResponse:
        chat_repos = {m.repository for m in _DOWNLOADABLE_CHAT_MODELS}
        embedding_repos = {m.repository for m in _DOWNLOADABLE_EMBEDDING_MODELS}
        ...
        asyncio.create_task(_download_background(repository, model_type))
    ...
    def load_model(self, model_path: str, model_type: str) -> ModelLoadResponse:
        resolved = _models_dir_for(model_type) / model_path.replace("/", os.sep)
