...
# ---------------------------------------------------------------------------
# Test 3 — Summarize big_text.txt using summary agent
# ---------------------------------------------------------------------------

def test3() -> bool:
    _header("Test 3 — Summarize big_text.txt (summary agent)")
    big_text_path = pathlib.Path("0_data/big_text.txt")
    text = big_text_path.read_text(encoding="utf-8")
    print(f"  Loaded {len(text):,} characters from {big_text_path}")

    reply = llm_service.agent_complete_chat(
        model_path=CHAT_MODEL,
        message_list=[
            _msg(Role.USER, f"Summarize the following text:\n\n{text}")
        ],
        agent=AgentFactory.summary(),
    )
    _result("3. Summary of big_text.txt", reply)
    return True
